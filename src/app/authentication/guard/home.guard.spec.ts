...
describe('canActivateGuardHome', () => {
  ...
  const run = () => TestBed.runInInjectionContext(() =>
    canActivateGuardHome({} as any, { url: '/dispatch' } as any)) as Observable<boolean | UrlTree>;
  beforeEach(() => {
  ...
  it('handles a denied or expired session', async () => {
    ...
    expect(await firstValueFrom(run())).toEqual(router.createUrlTree(['/login'], { queryParams: { returnUrl: '/dispatch' } }));
  });
